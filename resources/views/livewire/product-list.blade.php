        ...
        <div class="grid grid-cols-12 gap-6">
            ...
            @if(count($dinnerProducts))
                ...
                <div class="col-span-6 md:col-span-4 lg:col-span-4 xl:col-span-3 md:flex pb-8 w-full indicator">
                    <div class="card bordered shadow-lg w-full rounded-box bg-base-200">
                        <figure class="px-4 pt-4">
                            <img src="{{$dproduct->image_file? $dproduct->image_file : 'https://www.kenyons.com/wp-content/uploads/2017/04/default-image-620x600.jpg'}}" class="h-40 object-cover object-center rounded-box bg-cover bg-center">
                        </figure>
                        <div class="card-body h-30 px-5 pt-4 pb-0">
                            <span class="menu-title text-opacity-50 text-xs text-gray-800">
                                @php
                                ...
                                $sold = \App\Models\Order\OrderItem::where([
                                    'store_id' => $storeid,
                                    'menu_date' => $menu_date,
                                    'product_id' => $dproduct->id
                                ])->sum('quantity');
                                $stock = $dproduct->pivot->stock;
                                @endphp
                            </span>
                            <h4 class="font-bold text-xs lg:text-md">
                                {{$dproduct->title}}
                            </h4>
                            <p class="hidden lg:block text-xs mt-2">{{ mb_strimwidth($dproduct->description, 0, 50, "...") }}</p>
                        ...
                        <div class="pb-4 px-5 w-full mt-3 justify-between">
                            ...
                            @else
                                @auth
                                <button wire:click="addToCart({{$dproduct->id}}, {{$storeid}}, {{$dinnerPeriodId->id}}, '{{$menu_date}}')" class="btn btn-primary btn-block btn-sm text-sm m-0 rounded-lg">{{__('Add To Cart')}}</button>
                                @else
                                <a href="{{route('login')}}" class="btn btn-primary btn-block btn-sm text-sm m-0 rounded-lg">{{__('Add To Cart')}}</a>
                                @endauth
